...
As of the 2026-04-26 JST notice update, no separate upstream `LICENSE` file is vendored in this repository for `intfloat/multilingual-e5-small`; CYRUNE records the upstream model metadata and does not infer a missing upstream copyright notice. If upstream publishes or changes a separate license / notice file, this notice must be updated before relying on it for a later publication surface.
